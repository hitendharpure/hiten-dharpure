...
interface IbrmediaPageProps {
  onBack: () => void;
}

export default function IbrmediaPage({ onBack }: IbrmediaPageProps) {
  const mediaItems: { type: 'video' | 'image'; src: string; label: string; aspect: string; link?: string; linkText?: string }[] = [
    {
      type: 'image',
      src: './ibr/press.jpg',
      label: 'Newspaper Collage',
      aspect: 'aspect-auto',
    }
  ];
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2 text-wonder-green">
          ...
          <h3 className="text-xs font-bold uppercase tracking-widest">India Book of Records</h3>
        </div>

        <h1 className="text-3xl md:text-4xl font-black text-white leading-tight tracking-tight max-w-2xl">
          Media Coverage of India Book of Records
        </h1>
      </div>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          {/* Single Column Layout for Collage */}
