...
interface GpaimediaPageProps {
  onBack: () => void;
}

export default function GpaimediaPage({ onBack }: GpaimediaPageProps) {
  const mediaItems: { type: 'video' | 'image'; src: string; label: string; aspect: string; link?: string; linkText?: string; col?: 0 | 1 }[] = [
    {
      type: 'image',
      src: './gpai/media/1.jpg',
      label: 'The Hitavada - 21st December 2023',
      aspect: 'aspect-[5/8]',
      col: 0,
    },
    {
      type: 'image',
      src: './gpai/media/2.jpg',
      label: 'The Times of India - 22nd December 2023',
      aspect: 'aspect-[21/9]',
      col: 1,
    },
    {
      type: 'image',
      src: './gpai/media/3.jpg',
      label: 'GPAI Interview',
      aspect: 'aspect-[3/4]',
      col: 1,
    }
  ];
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2 text-wonder-green">
          ...
          <h3 className="text-xs font-bold uppercase tracking-widest">GPAI Summit Project</h3>
        </div>

        <h1 className="text-3xl md:text-4xl font-black text-white leading-tight tracking-tight max-w-2xl">
          Media Coverage of the GPAI Summit
        </h1>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-6 w-full">
            ...
            {mediaItems.length > 0 && [0, 1].map((colIndex) => (
              <div key={colIndex} className="flex flex-col gap-6 w-full md:w-1/2">
                {mediaItems.filter((item, i) => (item.col !== undefined ? item.col === colIndex : i % 2 === colIndex)).map((item, index) => (
