...
interface WeatherJourneyPageProps {
  onBack: () => void;
}
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-white leading-tight tracking-tight max-w-2xl">
          Complete journey to making an World smallest weather station
        </h1>
      </div>

      {/* Main Pallet: While Creating It */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {mediaItems.map((item, index) => (
              <div
                key={index}
                className="aspect-video w-full rounded-xl overflow-hidden bg-slate-950 border border-slate-800 hover:border-wonder-green/30 hover:shadow-xl transition-all duration-300 relative group"
              >
                {item.type === 'video' ? (
                  <video
                    src={item.src}
